...
import { DiamondIcon, LoaderCircle, Wallet, X } from "lucide-react";
...
const SCOPE_OPTIONS: {
...
}[] = [
  {
    value: "household",
    label: "Gabungan",
    description: `Menganalisa pengeluaran ${HOUSEHOLD_LABEL} digabung jadi satu.`,
  },
  {
    value: "own",
    label: "Punya saya",
    description: "Hanya menganalisa pengeluaran yang kamu catat sendiri.",
  },
];
...
export function LaporanPengeluaranView({ startDate, endDate }: Props) {
  const { user } = useAuth();
  ...
  return (
    <>
      <button
        type="button"
        aria-label="Analisa pengeluaran"
        onClick={openAnalyze}
        disabled={isLoading}
              ...
              {analyzeStep === "config" ? (
                <div className="flex flex-col gap-5">
                    ...
                    <label className="flex cursor-pointer select-none items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                          {analyzeComparePrev
                            ? `Membandingkan pengeluaran ${formatYearMonth(analyzeMonth)} vs ${formatYearMonth(getPrevYearMonth(analyzeMonth))}`
                            : `Membandingkan pengeluaran vs pemasukan di ${formatYearMonth(analyzeMonth)}`}
                        </p>
                      </div>
                    </label>
                  </div>
                  <div className="b-4 space-y-2">
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-4">
                      {
                        PERSONA_OPTIONS.find((o) => o.value === analyzePersona)
                          ?.description
                      }
                    </p>
                  </div>
                  <button
                    ...
                    className="mt-2 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-cyan-500 px-5 py-3 text-sm font-semibold text-white shadow-[0_8px_24px_rgba(6,182,212,0.35)] transition-all duration-300 ease-out hover:-translate-y-0.5 hover:bg-cyan-400 active:translate-y-0 dark:bg-cyan-600 dark:hover:bg-cyan-500"
                  >
                    <DiamondIcon className="h-4 w-4" />
                    Mulai Analisa
                  </button>
                ...
                <div className="flex min-h-60 flex-col items-center justify-center gap-6">
                  <div className="relative mb-6 flex items-center justify-center">
                    <span className="absolute h-16 w-16 animate-ping rounded-full bg-cyan-400/20 dark:bg-cyan-500/15" />
                    <span className="absolute h-10 w-10 animate-ping rounded-full bg-cyan-400/30 dark:bg-cyan-500/25 [animation-delay:200ms]" />
                    <LoaderCircle className="relative h-8 w-8 animate-spin text-cyan-500" />
                  </div>
                  <p className="text-sm font-medium text-slate-700 dark:text-slate-200">
                    Lagi ngitung-ngitung pengeluaranmu...
                  </p>
                </div>
              ) : analysisError ? (
                          ...
                          >
                            ...
                            {i === arr.length - 1 &&
                              typedResult.length < analysisResult.length && (
                                <span
                                  className="animate-blink ml-0.5 inline-block w-0.5 bg-cyan-500 align-middle"
                                  style={{ height: "1em" }}
                                />
                              )}{" "}
                            ~~
