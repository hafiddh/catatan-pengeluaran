...
export function ListPemasukanView({ startDate, endDate }: Props) {
  ...
  const [editIncomeType, setEditIncomeType] = useState<string>("");
  ...
  const incomeTypeMap = useMemo(
    () => new Map(incomeTypes.map((item) => [item.id, item])),
    [incomeTypes],
  );
  ...
  const fetchNotes = useCallback(
    async (page: number, append: boolean) => {
      ...
      try {
        const result = await listShoppingNotes({
          startDate,
          endDate,
  ...
  const openEditModal = (note: ShoppingNote) => {
    ...
    setEditIncomeType(note.kategori_id);
    setEditSumber(note.nama_barang ?? "");
  ...
  const handleUpdate = async () => {
    ...
    if (!editIncomeType) {
      showToast("Jenis pemasukan wajib dipilih", { type: "error" });
      return;
    }
    setIsSubmittingEdit(true);
    try {
      const updatedNote = await updateShoppingNote(editingNote.id, {
        tanggal: editDate,
        jumlah: parsedAmount,
        jenis_transaksi: "pemasukan",
        kategori_id: editIncomeType,
        nama_barang: editSumber || undefined,
      ...
      showToast(e instanceof Error ? e.message : "Gagal mengubah catatan", {
        type: "error",
      });
    } finally {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteShoppingNote(pendingDeleteNote.id);
      setNotes((current) =>
        current.filter((item) => item.id !== pendingDeleteNote.id),
      );
      if (editingNote?.id === pendingDeleteNote.id) closeEditModal();
      setPendingDeleteNote(null);
      showToast("Catatan berhasil dihapus", { type: "success" });
    } catch (e: unknown) {
      showToast(e instanceof Error ? e.message : "Gagal menghapus catatan", {
        type: "error",
      });
    } finally {
      setDeletingId("");
    ...
    <>
        ...
        {isLoading ? (
          ...
          <>
            <div className="divide-y divide-gray-100 dark:divide-slate-800 md:hidden">
              {notes.map((note) => {
                const category = incomeTypeMap.get(note.kategori_id);
                return (
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-5 py-4 align-top sm:px-6">
                          <div className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3.5 py-1.5 text-sm font-semibold text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-200">
                            {getExpenseTypeIcon(
                              category?.icon,
                              category?.label || "Tanpa kategori",
                            )}
      ...
      {editingNote ? (
        <div className="-mt-10 fixed inset-0 z-80 flex items-center justify-center bg-slate-950/45 px-3 backdrop-blur-sm">
          ...
          <section className="relative z-10 mb-10 max-h-[80vh] w-full max-w-2xl overflow-auto rounded-3xl border border-white/45 bg-white/35 p-3 shadow-[0_24px_60px_rgba(15,23,42,0.22)] backdrop-blur-md dark:border-slate-700/70 dark:bg-slate-900/40 sm:p-6">
            <div className="grid grid-cols-1 gap-4">
              ...
              <label className="space-y-3">
                ...
                <AmountInput value={editAmount} onChange={setEditAmount} min={0} />
              </label>
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1 text-sm text-gray-500 dark:text-slate-300">
                  ...
                  <span className="font-medium text-gray-900 dark:text-slate-100">
                    {incomeTypeMap.get(pendingDeleteNote.kategori_id)?.label ||
                      "tanpa kategori"}
                  </span>{" "}
                  pada tanggal{" "}
                  <span className="font-medium text-gray-900 dark:text-slate-100">
